...
- **Default avatars:** 11 built-in SVGs in `client/public/avatars/`, whitelisted by exact path in `DEFAULT_AVATARS` (`@taking5/shared`) — server validation accepts only these relative paths (plus http(s) URLs and png/jpeg/webp data URLs). Players pick one via `AvatarPickerModal` (or upload their own); a player joining without an avatar gets a sticky random default assigned client-side in `useRoomSocket`'s `roomJoined` handler. Explicit removal is remembered as `take5_avatarUrl = ""` in localStorage, so removed stays removed across joins; only users who never chose (key absent) get a random assignment.
