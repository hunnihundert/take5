...
- **Voluntary disconnect:** Client emits `leaveRoom` on `beforeunload`; server removes the player immediately (skips grace period) if no other tabs for that session are open
